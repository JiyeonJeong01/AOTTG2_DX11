...
void IScript::Save_Exposed_Fields(json& j) const
{
    ...
    for (const SCRIPT_FIELD_DESC& tDesc : pInfo->vecFields)
    {
        ...
        case SCRIPT_FIELD_TYPE::CHAR32:
            ScriptField_ToJson_Char32(jField["Value"], reinterpret_cast<const char*>(pField));
            break;
...
_bool IScript::Load_Exposed_Fields(const json& j)
{
    ...
    for (const json& jField : jFields)
    {
        ...
        case SCRIPT_FIELD_TYPE::CHAR32:
            ScriptField_FromJson_Char32(jValue, reinterpret_cast<char*>(pField));
            break;

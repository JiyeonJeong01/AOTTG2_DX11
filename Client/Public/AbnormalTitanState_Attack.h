#pragma once

#include "TitanState.h"

NS_BEGIN(Client)

class CAbnormalTitanState_Attack final : public CTitanState
{
public:
    CAbnormalTitanState_Attack(Engine::CGameObject* goTitan, CTitan* scTitan, TITAN_STATE eState);
    ~CAbnormalTitanState_Attack();

public:
    HRESULT Initialize() override;
    void    Priority_Update(_float fDT) override;
    void    Update(_float fDT) override;
    void    Late_Update(_float fDT) override;

    void    Enter(_uint iDetailFlag) override;
    void    Exit() override;

    void    Setup_CachedTitanContext() override;

    _uint   Get_DetailState() const override;

private:
    void    Decide_NextState() override;
    void    Decide_NextAnim() override;

private:
    TITAN_MOVE  m_eMoveState = TITAN_MOVE::WALK;
    _float3     m_vAttackDir = {};
    _float      m_fAttackDist = {};

public:
    static std::shared_ptr<CAbnormalTitanState_Attack> Create(Engine::CGameObject* goTitan, CTitan* scTitan, TITAN_STATE eState);
};

NS_END

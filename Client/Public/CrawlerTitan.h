...
NS_BEGIN(Client)
class CCrawlerTitanStateMachine;
class CTargetSensor;
class CTitanState;
class CHurtBox;
class CVFX_Manager;
...
class CCrawlerTitan : public IScript, public CTitan
{
public:
    CCrawlerTitan();
    ~CCrawlerTitan();
...
private:
    ...
    std::unique_ptr<CCrawlerTitanStateMachine>      m_upStateMachine{};
...
private:
    ...
    void On_Hurt(const HIT_INFO& tHitInfo, const std::string& strHurtBox);
